                ...
                .then((serverArr) => {
                    ...
                    backup.createBackupPrePostCommands('postrun', self.bServer, self.eServer, self.bConfig) //check and create the postrun comands for a backup server.
                        .then((backupPostRun) => {
                            ...
                            ssh.copySSHKey(self.eServer, self.bConfig.SSHKey, self.bConfig) //copy ssh key to exec server
                                .then((commandStat) => {

                                    backup.runCommands(toRunArr, self.eServer, self.bServer, self.bConfig) //run backups of all object in array "toRunArr"
                                        .then((outArr) => {

                                            ssh.deleteSSHKey(self.eServer, self.bConfig.SSHKey, self.bConfig) //delete ssh key after all backups
                                                .then((out)=>{
