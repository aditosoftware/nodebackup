...
var sshRun = require('../misc/execArray');
...
var createBackupCommands = async function (serverConfig, server, dockerType, backupConfig, backupServer, execServer) {
    ...
    if (config.sftpServer !== undefined && config.sftpServer !== null) {
        if (config.sftpServer.sudo !== undefined && config.sftpServer.sudo !== null && config.sftpServer.sudo === true) {
            if (config.sftpServer.path !== undefined && config.sftpServer.path !== null) {
                var sshParam = ' -o ServerAliveInterval=15 -o StrictHostKeyChecking=no -o StrictHostKeyChecking=no -o Compression=no -o IdentityFile=/tmp/' + idkey + ' -o sftp_server="/usr/bin/sudo ' + config.sftpServer.path + '"';
            } else {
                logger.debug("sfptServer.sudo set to true, but sftpServer.path is not defined, ignore");
                var sshParam = ' -o IdentityFile=/tmp/' + idkey + ' -o ServerAliveInterval=15 -o StrictHostKeyChecking=no -o Compression=no';
            }
        } else {
            var sshParam = ' -o IdentityFile=/tmp/' + idkey + ' -o StrictHostKeyChecking=no -o ServerAliveInterval=15 -o Compression=no';
        }
    } else {
        var sshParam = ' -o IdentityFile=/tmp/' + idkey + ' -o StrictHostKeyChecking=no -o ServerAliveInterval=15 -o Compression=no';
    }
    ...
    if (config.SMB !== undefined) {
        if(config.SMB.SMBVERS != undefined && config.SMB.SMBVERS != null){
          var smbvers = ',vers=' + config.SMB.SMBVERS;
        } else {
          var smbvers = '';
        }
        if (config.SMB.SET === "true") {
            if (config.SMB.DOMAIN !== undefined && config.SMB.DOMAIN !== null) {
                var mComandTemp = "sudo mount -t cifs //" + config.HOST + "/" + config.SMB.PATH + " /tmp/" + cName + " -o username=" + config.USER + ",password=\'" + config.SMB.PASS + "\',domain=" + config.SMB.DOMAIN + ",iocharset=utf8,file_mode=0777,dir_mode=0777" + smbvers;
            } else {
                var mComandTemp = "sudo mount -t cifs //" + config.HOST + "/" + config.SMB.PATH + " /tmp/" + cName + " -o username=" + config.USER + ",password=\'" + config.SMB.PASS + "\',iocharset=utf8,file_mode=0777,dir_mode=0777" + smbvers;
            }
    ...
    if (postRunCommands !== undefined && postRunCommands !== null && postRunCommands.length > 0) { //if postrun commands exist add this to command arr -> commandArr;
        ...
        //logger.debug(sName + ":postRun found");
    }
