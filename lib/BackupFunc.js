...
var dupl = require('./duplicityCommands');
var borg = require('./borgCommands');
    ...
    if (backupProv == 'borg') {
        ...
        for (var y = 0; y < servers.length; y++) {
            ...
            if (config.kube !== undefined && config.kube !== null) {//this is a kubernetes node
                ...
                var kubeDeploys = await borg.getAllDeployOnKube(servers[y], backupConfig, backupServer, execServer);

                for (var i = 0; i < kubeDeploys.length; i++) { //create backupcommands for each deploy, where backup annotation are defined
                    var depComm = await dupl.createBackupCommands(kubeDeploys[i], servers[y], true, backupConfig, backupServer, execServer);
