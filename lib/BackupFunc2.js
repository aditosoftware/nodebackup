...
backupRun.prototype.createBackupCommands = async function (serverConfig, server, dockerType) {
    //    serverconfig =  { HOST: 'superdocker.aditosoftware.local',
    //   USER: 'docker',
    //   PORT: 22,
    ...
    if (config.passphrase !== undefined && config.passphrase !== null) {
        ...
        self.logger.debug("Passphrase for backup is in container/server defiened");
    } else {
    ...
    if(config.noffullbackup != undefined){
        ...
        var duplicityRunClean = duplicityComm + " remove-all-but-n-full --force " + num + rsyncParam + duplicityRsyncSudo + duplicityArch;
        var runDupFullBackupClean = {
            'server': self.bServer,
            'command': duplicityRunClean,
            'stat': false
        }
        self.logger.debug(sName + ": " + runDupFullBackupClean);
        ConfigArr.push(runDupFullBackupClean);

    } else {
        self.logger.debug(sName + ": Command to remove old Backups was not found");
    }
