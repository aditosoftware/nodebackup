            ...
            for (var y = 0; y < commands.length; y++) {
                ...
                try {
                    ...
                    if (outputObj.server == self.bServer) {
                        ...
                        if (failover !== undefined && failover !== null && failover !== false) {
                            try {
                                var outputComm = await self.runUnderSSH(failover, self.sshkey, eServer, tServer);
                                outputObj.output.message = "Command run error: " + err + ". Run failover comand: Output: " + outputComm;
                                outputArr.push(outputObj);
                            }
                            catch (err) {
...
backupRun.prototype.getAllContainerOnServer = async function (server) {
    ...
    var dockerOutput = await self.runUnderSSH(getContainers, self.sshkey, execConfig, dockerHostConfig);
...
backupRun.prototype.containerProm = async function (container, dockerHost) {
    ...
    if (labels.backup !== undefined && labels.backup !== null) {
        ...
        var getLinksOfCon = await self.runUnderSSH(getContainerInfo, self.sshkey, execConfig, dockerHostConfig);
        ...
        if (labels.startaftererror !== undefined && labels.startaftererror !== null) {
            if (labels.startaftererror == "false") {
                ...
                serverConnObj.failover = dockerstart
            }
        } else {
            serverConnObj.startaftererror = true;
            serverConnObj.failover = dockerstart
        ...
        if(labels.failovercustom !== undefined && labels.failovercustom !== null){
            serverConnObj.failover = labels.failovercustom;
        }
...
//   PORT: '22' 
// }
backupRun.prototype.runUnderSSH = async function (command, sshkey, server1, server2) {
    var self = this;
