...
var createBackupCommands = async function (serverConfig, server, dockerType, backupConfig, backupServer, execServer) {
    ...
    if (InExclude !== undefined) {
        ...
        var borgRunTemp = passphrase + cacheDir + cacheDir + ' borg create' + compression + ' --json ' + backupServerConfig.backuppath + '/' + cName + '::' + dateformated + '_' + randNum + ' /tmp/' + cName;
    }
    ...
    logger.debug('Backup respository was not found, inizialise one new');
    ...
    if (config.keepbackup !== undefined && config.keepbackup !== null) {
        logger.debug(cName + ": Next full backup will be start after: " + backupConfig.keepbackup);
