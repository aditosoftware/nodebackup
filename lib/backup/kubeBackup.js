    ...
    if (allDeploys.length > 0) {
        ...
        for (var i = 0; i < allDeploys.length; i++) {
            ...
            if (annot.prerun !== undefined && annot.prerun !== null) { //check preruns
                prerun = JSON.parse(annot.prerun);
            }

            if (annot.postrun !== undefined && annot.postrun !== null) { //check postruns
                postrun = JSON.parse(annot.postrun);
            }
            ...
            if (annot.strategy == "off") { //get type of backup (shutdown bevor backup or not)
                ...
                deployDel = 'kubectl delete -n ' + deployNs + ' deploy ' + deployName + ' --grace-period=60';
                deployPause = 'sleep 60'; //linux command to wait 60 sec.
                deployCrt = "echo '" + JSON.stringify(deploy) + "' | kubectl create -n " + deployNs + " -f -";
            }
